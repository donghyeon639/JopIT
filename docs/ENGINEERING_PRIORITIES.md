...
| `@Async`, AI 호출, `LlmPort`/`BedrockLlmService`, AI 피드백 파이프라인, 중복 호출/비용 방어 | §3 비동기 AI 처리 |
...
- 현재 `BedrockLlmService`(AWS Bedrock, Converse API)는 단발 `generate()`만 구현(스트리밍 미지원) → **화상 면접/실시간 피드백 착수 시점에 `LlmPort`에 스트리밍(Bedrock Converse 스트리밍) 추가 필수**(도메인은 `LlmPort`만 의존하므로 어댑터만 확장).
...
### 본 프로젝트 점검 대상
- `BedrockLlmService`가 "호출 시작/완료·소요시간"은 찍지만 **답변 ID·사용자 ID 컨텍스트 없음** → MDC 도입 후 자연스럽게 묶이도록.
