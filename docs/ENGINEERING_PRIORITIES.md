...
| `@Async`, AI 호출, `LlmPort`/`ClaudeCliService`, AI 피드백 파이프라인, 중복 호출/비용 방어 | §3 비동기 AI 처리 |
...
- 현재 `ClaudeCliService`는 매 호출마다 `npx -y @anthropic-ai/claude-code` 콜드 스타트로 5~10초 지연 + 스트리밍 불가 → **화상 면접/실시간 피드백 착수 시점에 스트리밍 가능한 `LlmPort` 구현체로 교체 필수**(SDK 또는 로컬 LLM 등 — 도메인은 `LlmPort`만 의존하므로 어댑터만 교체).
